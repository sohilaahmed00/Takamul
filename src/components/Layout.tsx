...
const SidebarItem = ({ icon: Icon, label, active, hasSubmenu, isOpen, isSidebarOpen = true, onClick }: SidebarItemProps) => {
  return (
    <button onClick={onClick} className={cn("w-full flex items-center justify-between p-3 rounded-lg transition-colors duration-200", active ? "bg-[var(--primary)] text-white shadow-md" : "text-[var(--text-main)] hover:bg-[var(--bg-main)] hover:text-[var(--primary)]", !isSidebarOpen && "justify-center")} title={!isSidebarOpen ? label : undefined}>
  ...
  const SubmenuItem = ({ label, icon: Icon, path, state, onClick }: SubmenuItemProps) => {
    ...
    return (
      <button
        ...
        className={cn("w-full p-2 text-sm rounded-md flex items-center gap-2 transition-colors group", isActive ? "bg-[var(--primary)] text-white" : "text-[var(--text-main)] hover:text-[var(--primary)] hover:bg-[var(--bg-main)]", direction === "rtl" ? "text-right" : "text-left")}
      >
    ...
    <div className="min-h-screen flex transition-colors duration-300" dir={direction}>
      ...
      >
        ...
        <div className="p-3 space-y-1">
          ...
          <SidebarItem icon={CornerUpLeft} label={t("purchases")} hasSubmenu isSidebarOpen={showSidebarContent} isOpen={openSubmenu === "purchases"} onClick={() => toggleSubmenu("purchases")} />
          <AnimatePresence>
          ...
          <SidebarItem icon={Users} label={t("users")} hasSubmenu isSidebarOpen={showSidebarContent} isOpen={openSubmenu === "users"} onClick={() => toggleSubmenu("users")} />
          <AnimatePresence>
          ...
          <SidebarItem icon={User} label={t("customers")} hasSubmenu isSidebarOpen={showSidebarContent} isOpen={openSubmenu === "customers"} onClick={() => toggleSubmenu("customers")} />
          <AnimatePresence>
          ...
          <SidebarItem icon={Truck} label={t("suppliers")} hasSubmenu isSidebarOpen={showSidebarContent} isOpen={openSubmenu === "suppliers"} onClick={() => toggleSubmenu("suppliers")} />
          <AnimatePresence>
          ...
          <SidebarItem icon={Landmark} label="الخزائن" hasSubmenu isSidebarOpen={showSidebarContent} isOpen={openSubmenu === "treasurys"} onClick={() => toggleSubmenu("treasurys")} />

          <AnimatePresence>
            {openSubmenu === "treasurys" && showSidebarContent && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className={cn("overflow-hidden space-y-1 pr-2", direction === "rtl" ? "mr-4 border-r border-gray-100" : "ml-4 border-l border-gray-100 pl-2 pr-0")}>
                <SubmenuItem label="قائمة الخزائن" icon={List} path="/treasurys" />
                <SubmenuItem label="كشف حساب خزينة" icon={ArrowUpRight} path="/treasury/external-transfers" />
                <SubmenuItem label="تحويلات داخلية" icon={ArrowLeftRight} path="/treasury/internal-transfers" />
              </motion.div>
            )}
          </AnimatePresence>
          <SidebarItem icon={Building} label={t("branches")} hasSubmenu isSidebarOpen={showSidebarContent} isOpen={openSubmenu === "branches"} onClick={() => toggleSubmenu("branches")} />
          <AnimatePresence>
          ...
          {/* warehouses */}
          <SidebarItem icon={Store} label={t("warehouses")} hasSubmenu isSidebarOpen={showSidebarContent} isOpen={openSubmenu === "warehouses"} onClick={() => toggleSubmenu("warehouses")} />
          <AnimatePresence>
        ...
        <header ref={headerRef} data-theme={theme} className="bg-[var(--bg-card)] h-16 border-b border-[var(--border)] flex items-center justify-between px-4 lg:px-8 shadow-sm z-10 transition-colors duration-300">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden md:flex items-center gap-1">
              <button onClick={() => navigate("/sales/create")} className="flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-full transition-all duration-200 hover:shadow-sm active:scale-95">
                <LayoutGrid size={13} />
                <span>{t("sales_a4_quick")}</span>
              </button>

              <button onClick={() => navigate("/sales/pos")} className="flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-full transition-all duration-200 hover:shadow-sm active:scale-95">
                <ShoppingCart size={13} />
                <span>{t("pos_quick")}</span>
              </button>

              <button onClick={() => navigate("/sales/all")} className="flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium bg-violet-50 text-violet-700 hover:bg-violet-100 rounded-full transition-all duration-200 hover:shadow-sm active:scale-95">
                <List size={13} />
                <span>{t("all_sales")}</span>
              </button>

              <button onClick={() => navigate("/products/create")} className="flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium bg-amber-50 text-amber-700 hover:bg-amber-100 rounded-full transition-all duration-200 hover:shadow-sm active:scale-95">
                <Package size={13} />
