...
const AddGroupModal: React.FC<AddGroupModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleAddGroup = async (e: React.FormEvent) => {
    ...
    try {
      await addGroup(groupName.trim(), groupNameSecondary.trim() || undefined, description.trim() || undefined, groupNameUr.trim() || undefined, imageFile || undefined);
      ...
      const msg = err instanceof Error ? err.message : direction === "rtl" ? "فشل إضافة التصنيف" : "Failed to add category";

      setToastMessage(direction === "rtl" ? `فشل إضافة التصنيف: ${msg}` : `Failed to add category: ${msg}`);
      setToastType("error");
      setShowToast(true);
    } finally {
      setSubmitting(false);
  ...
  const toastElement = <Toast isOpen={showToast} message={toastMessage} type={toastType} onClose={() => setShowToast(false)} />;
