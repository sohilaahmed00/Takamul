...
export default function AddExpenseCategoryModal({ isOpen, onClose, category }: AddExpenseCategoryModalProps) {
  ...
  return (
    <ResponsiveModal isOpen={isOpen} onClose={onClose} title={category ? t("edit") : t("add_expense_category")} maxWidth="max-w-md" description={direction === "rtl" ? "برجاء ادخال المعلومات أدناه. تسميات الحقول التي تحمل علامة * هي حقول اجبارية ." : "Please enter the information below. Field labels marked with * are mandatory."}>
      <div className="p-6" dir={direction}>
        <p className="text-sm text-gray-600 mb-6">{direction === "rtl" ? "برجاء ادخال المعلومات أدناه. تسميات الحقول التي تحمل علامة * هي حقول اجبارية ." : "Please enter the information below. Field labels marked with * are mandatory."}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-fit px-6 py-2 bg-[#004d3d] hover:bg-[#003d30] text-white rounded-lg font-bold transition-colors mt-4">
            {category ? t("update") : t("add_expense_category")}
          </button>
        </form>
      </div>
    </ResponsiveModal>
