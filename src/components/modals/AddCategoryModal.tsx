...
const AddCategoryModal: React.FC<AddCategoryModalProps> = ({ isOpen, onClose, category }) => {
  ...
  useEffect(() => {
    if (category) {
      setFormData({
        ...
        branchAvailability: category.branchAvailability || [{ branchName: "تجريبي", status: true }],
      });
  ...
  return (
    <ResponsiveModal isOpen={isOpen} onClose={onClose} title={category ? t("edit_category") : t("add_category")} maxWidth="max-w-2xl">
      <form onSubmit={handleSubmit} className="space-y-6 p-6" dir={direction}>
        <div className="text-[var(--text-muted)] text-sm text-center">{t("mandatory_fields")}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            <label className="takamol-label">{t("category_image")}</label>
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => imageInputRef.current?.click()} className="btn-primary !px-4 !py-2 !text-sm whitespace-nowrap">
                <Upload size={16} />
                {t("browse")}
              </button>
          ...
          <div className="space-y-2">
            <label className="takamol-label">{t("category_banner")}</label>
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => bannerInputRef.current?.click()} className="btn-primary !px-4 !py-2 !text-sm whitespace-nowrap">
                <Upload size={16} />
                {t("browse")}
              </button>
        ...
        <div className="takamol-card overflow-hidden">
          <div className="bg-[var(--primary)] p-3 text-white text-center font-bold text-sm">{t("category_availability")}</div>
