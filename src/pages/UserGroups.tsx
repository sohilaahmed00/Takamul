...
import { useUserGroups, UserGroup, Permission } from '@/context/UserGroupsContext';
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-sm overflow-hidden">
          <div className="bg-table-header p-3 flex items-center justify-between text-white">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-4 py-1.5 bg-white/20 hover:bg-white/30 text-white rounded transition-colors text-sm font-bold shadow-sm"
            >
              <Save size={18} />
              تحديث
            </button>
                  ...
                  {modules.map((module) => (
                    <tr key={module.id} className="hover:bg-table-row-hover transition-colors border-b border-gray-100 dark:border-border">
                      ...
                      <td className="p-3 border-x border-gray-100 dark:border-border text-center">
                        <input 
                          type="checkbox" 
                          checked={selectedGroup.permissions[module.id]?.view || false} 
                      ...
                      <td className="p-3 border-x border-gray-100 dark:border-border text-center">
                        <input 
                          type="checkbox" 
                          checked={selectedGroup.permissions[module.id]?.add || false} 
                      ...
                      <td className="p-3 border-x border-gray-100 dark:border-border text-center">
                        <input 
                          type="checkbox" 
                          checked={selectedGroup.permissions[module.id]?.edit || false} 
                      ...
                      <td className="p-3 border-x border-gray-100 dark:border-border text-center">
                        <input 
                          type="checkbox" 
                          checked={selectedGroup.permissions[module.id]?.delete || false} 
  ...
  return (
    <div className="p-4 space-y-4" dir={direction}>
      ...
      <DeleteConfirmationModal
        ...
        itemName={userGroups.find(g => g.id === groupToDelete)?.name}
      />
                    ...
                    <tr key={`desktop-${group.id}`} className="hover:bg-primary/5 transition-colors border-b border-gray-100 dark:border-gray-700">
                      ...
                      <td className="p-3 border-r border-gray-100 dark:border-gray-700 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button 
                            onClick={() => handlePermissionsClick(group)}
                            className="p-1.5 text-orange-600 hover:bg-orange-50 dark:hover:bg-orange-900/20 rounded-lg transition-colors border border-orange-100 dark:border-orange-900/30"
                            title={t('permissions')}
                          >
                            <Shield size={16} />
                          </button>
              ...
              filteredGroups.map((group) => (
                <div key={`mobile-${group.id}`} className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm space-y-3">
                  ...
                  <div className="flex justify-end gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                    <button 
                      onClick={() => handlePermissionsClick(group)}
                      className="p-2 text-orange-600 hover:bg-orange-50 rounded-lg border border-orange-100 transition-colors flex items-center gap-1 text-xs font-bold"
                    >
                      <Shield size={16} />
                      {t('permissions')}
                    </button>
      ...
      <ResponsiveModal
        ...
        footer={
          <button 
            ...
            className="w-full py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors font-bold shadow-sm"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1 text-right">
            ...
            <input 
              ...
              className="w-full p-2 border border-gray-300 rounded-lg outline-none focus:border-emerald-500 bg-white text-right"
            />
          </div>
