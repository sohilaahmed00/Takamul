...
export default function EditPurchase() {
  ...
  useEffect(() => {
    const purchaseToEdit = initialPurchases.find((p) => p.id === id);
    if (purchaseToEdit) {
      setFormData({
        date: purchaseToEdit.date,
        refNo: purchaseToEdit.reference,
        purchaseType: "warehouse",
        ...
        discountBeforeVat: 0,
        discountAfterVat: 0,
        amountPaid: purchaseToEdit.paid,
        paymentType: purchaseToEdit.paymentStatus === PaymentStatus.PAID ? "cash" : "credit",
        paymentTerms: "",
        notes: "",
      });
    }
  ...
  const filteredProducts = searchQuery.trim() === "" ? [] : allProducts.filter((p) => p.name.toLowerCase().includes(searchQuery.toLowerCase()) || p.code.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const handleSelectProduct = (product: Product) => {
    ...
    if (existingItemIndex !== -1) {
      ...
      const newItem: PurchaseItem = {
        ...
        vatAmount: vatAmount,
        totalSr: totalSr,
        publicPrice: parseFloat(String(product.price)) || 0,
      };
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 pb-20">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-4 bg-gray-50 border-b border-gray-100">
          <p className="text-sm text-gray-600">{t("edit_product_desc")}</p>
        </div>
        ...
        <form onSubmit={handleComplete} className="p-6 space-y-8">
          ...
          <div className="bg-orange-50/30 p-4 rounded-lg border border-orange-100 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-gray-700">{t("expected_profit")}</label>
                <div className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm bg-gray-50 text-center font-bold">0</div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <h3 className="text-sm font-bold text-gray-700">{t("items")}</h3>
              ...
              <table className="w-full text-sm text-right border-collapse">
                <thead>
                  <tr className="bg-primary text-white">
                    ...
                    <th className="p-3 border border-primary-hover">{t("expiry_date")}</th>
                    <th className="p-3 border border-primary-hover">{t("unit_cost")}</th>
                    <th className="p-3 border border-primary-hover">{t("quantity")}</th>
                    <th className="p-3 border border-primary-hover">{t("free")}</th>
                    <th className="p-3 border border-primary-hover">{t("total_no_tax")}</th>
                    <th className="p-3 border border-primary-hover">{t("vat_rate")}</th>
                    <th className="p-3 border border-primary-hover">{t("item_vat")}</th>
                    <th className="p-3 border border-primary-hover">{t("total_product_sr")}</th>
                    <th className="p-3 border border-primary-hover">{t("public_price")}</th>
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <div className="border border-gray-300 rounded-md overflow-hidden">
                  ...
                  <textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} className="w-full p-3 h-40 outline-none resize-none text-sm" placeholder={t("add_notes_here")} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-6">
            ...
            <button type="button" onClick={handleReset} className="bg-red-500 text-white px-8 py-2.5 rounded-md font-bold hover:bg-red-600 transition-all shadow-md">
              {t("reset_form")}
            </button>
          </div>
        </form>
