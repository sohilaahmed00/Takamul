...
const CreateReturnPurchasesInvoiceSchema = (t: (key: string) => string) =>
  z.object({
...
type PurchaseInvoiceType = z.infer<ReturnType<typeof CreateReturnPurchasesInvoiceSchema>>;

const CreateReturnPurchasesInvoice: React.FC = () => {
  const { t, direction } = useLanguage();
  const navigate = useNavigate();

  const purchasesInvoiceSchema = useMemo(() => CreateReturnPurchasesInvoiceSchema(t), [t]);
  ...
  return (
    <Card dir={direction}>
      <CardHeader>
        <CardTitle> فاتورة مرتجع مشتريات</CardTitle>
        <CardAction>
                ...
                <div key={payment.id} className="flex flex-col sm:flex-row gap-3 items-start bg-zinc-50 dark:bg-zinc-900/40 p-3 rounded-lg border border-zinc-100 dark:border-zinc-800">
                  ...
                  <div className="w-full flex-1">
                    ...
                    <Controller
                      ...
                      render={({ field, fieldState }) => (
                        <Field>
                          <ComboboxField field={field} items={treasurys} valueKey="id" labelKey="name" placeholder={t("choose")} className="bg-white" />
                          {fieldState?.error && <FieldError errors={[fieldState.error]} />}
                        </Field>
                      )}
                    />
...
export default CreateReturnPurchasesInvoice;
