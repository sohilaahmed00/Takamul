...
export default function AddProduct() {
  ...
  const filteredParentProducts = useMemo(() => {
    return directProductsList.filter((p) =>
  ...
  const filteredMaterials = useMemo(() => {
    return materialsProductsList.filter((m) =>
  ...
  const loadSubCategories = async (mainCategoryId: string) => {
    ...
    try {
      ...
      const data = await fetchJson<ApiCategory[]>(
        `${API_BASE}/api/ProductCategories/SubCategory/${encodeURIComponent(mainCategoryId)}`,
        {
          method: 'GET',
          headers: { accept: '*/*' },
        }
  ...
  const loadProductForEdit = async (productId: string) => {
    try {
      ...
      const productName =
        data?.productNameAr || data?.productNameEn || data?.productNameUr || '';
      ...
      setFormData((prev) => ({
        ...
        subCategoryId: '',
        productNature:
          location.state?.productNature ||
          (String(data?.productType || '').toLowerCase() === 'prepared'
            ? 'prepared'
            : String(data?.productType || '').toLowerCase() === 'branched'
            ? 'sub'
            : 'basic'),
        parentProductIds: Array.isArray(data?.parentProductIds)
          ? data.parentProductIds.map(String)
          : data?.parentProductId
          ? [String(data.parentProductId)]
          : [],
        parentProductNames: Array.isArray(data?.parentProductNames)
          ? data.parentProductNames.map(String)
          : data?.parentProductName
          ? [String(data.parentProductName)]
          : [],
  ...
  const handleAddMaterial = () => {
    ...
    const selectedMaterial = materialsProductsList.find(
      (m) => m.id === materialSearch || m.name === materialSearch
    );

    if (!selectedMaterial) return;

    const alreadyExists = formData.materials.some(
      (m) => String(m.materialId) === String(selectedMaterial.id)
    );
  ...
  const buildFormDataForUpdate = () => {
    ...
    const categoryNameToSend =
      getCategoryNameToSend(sub || main) || formData.category || '';
    ...
    fd.append(
      ...
      String(
        Number(
          formData.productNature === 'prepared' ? preparedCost : formData.cost || 0
        )
      )
    );
    fd.append('SellingPrice', String(Number(formData.sellingPrice || 0)));
    ...
    fd.append(
      'ParentProductId',
      formData.productNature === 'sub' && formData.parentProductIds.length > 0
        ? String(formData.parentProductIds[0])
        : '0'
    );
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!formData.nameLang2.trim()) {
      newErrors.nameLang2 =
        direction === 'rtl' ? 'الاسم باللغة الثانية مطلوب' : 'Second language name is required';
    }

    if (!formData.nameLang3.trim()) {
      newErrors.nameLang3 =
        direction === 'rtl' ? 'الاسم باللغة الثالثة مطلوب' : 'Third language name is required';
    }
    ...
    if (
      ...
      (formData.cost.trim() === '' || isNaN(Number(formData.cost)))
    ) {
    ...
    if (
      ...
      newErrors.sellingPrice =
        direction === 'rtl' ? 'سعر البيع غير صحيح' : 'Invalid selling price';
    }
    ...
    try {
      ...
      console.error(err);
      alert(
        (direction === 'rtl' ? 'فشل حفظ الصنف: ' : 'Failed to save product: ') +
          (err?.message || '')
      );
    }
  };
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h1 className="text-xl font-bold text-gray-800 flex items-center gap-2 mb-2">
          ...
          {isEditMode
            ...
            : `إضافة ${getProductNatureLabel(formData.productNature)}`}
        </h1>
      </div>
          ...
          <div className="bg-blue-50/50 p-6 rounded-xl border border-blue-100">
            ...
            <div className="flex flex-wrap gap-4">
              {(['basic', 'sub', 'prepared', 'materials'] as const).map((nature) => (
                <label
                  key={nature}
                  className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all cursor-pointer min-w-[180px] ${
                    formData.productNature === nature
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-6">
            <div className="space-y-5">
              ...
              <div>
                ...
                />
                {errors.nameLang2 && (
                  <p className="mt-1 text-xs text-red-600">{errors.nameLang2}</p>
                )}
              </div>
              ...
              <div>
                ...
                />
                {errors.nameLang3 && (
                  <p className="mt-1 text-xs text-red-600">{errors.nameLang3}</p>
                )}
              </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      onClick={() =>
                        setFormData((prev) => ({
                          ...prev,
                          code: `${systemSettings?.prefixes?.product || 'PRD'}${Math.floor(
              ...
              {formData.productNature === 'prepared' && (
                <div>
                  ...
                  <input
                    type="number"
                    value={preparedCost}
                    className="takamol-input font-bold text-[var(--primary)] bg-gray-50"
                    readOnly
                  />
                </div>
              )}
                            ...
                            {filteredParentProducts.map((product) => {
                              ...
                              return (
                                <div
                                  ...
                                  className={`p-2 hover:bg-gray-50 cursor-pointer border-b border-gray-100 last:border-0 flex items-center justify-between ${
            ...
            <div className="space-y-5">
              ...
              {formData.productNature !== 'materials' && (
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  <label
                    htmlFor="hideInPos"
                    className="text-sm font-bold text-gray-700 cursor-pointer flex-1"
                  >
                    إخفاء في نقاط البيع
                  </label>
                </div>
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">الوصف <span className="text-red-500">*</span></label>
                <textarea
                  name="details"
                  value={formData.details}
                  onChange={handleInputChange}
                  className="takamol-input w-full min-h-[120px]"
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">
                  صورة {getProductNatureLabel(formData.productNature)}
                </label>
            ...
            <div className="lg:col-span-2">
              <div className="text-xs text-gray-500">
                {isLoadingProducts
                  ? 'جاري تحميل الأصناف المرتبطة...'
                  : `عدد الأصناف المحملة: ${allProducts.length}`}
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-gray-100">
            ...
            <button
              ...
              className={cn(
                ...
                isEditMode ? 'bg-blue-600 hover:bg-blue-700' : 'bg-[var(--primary)] hover:bg-[var(--primary-hover)]'
              )}
            >
