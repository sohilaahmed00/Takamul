...
import {
    Plus, Eye, Pencil, Trash2, Minus, Asterisk, X, Printer, Search,
    Clock, Calendar, Building, User, ShoppingCart, Layers, ChevronDown, PanelLeftClose, PanelLeftOpen, LayoutGrid, CreditCard, Save
} from 'lucide-react';
...
export default function POS() {
    ...
    const searchRefBlue = useRef<HTMLDivElement>(null); // للبحث في الشاشة الزرقاء
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (searchRefBlue.current && !searchRefBlue.current.contains(event.target as Node)) {
                setShowSearchDropdown(false);
            }
        };
    ...
    const categories = useMemo(() => {
        const cats = directProducts.map(p => p.category).filter(Boolean);
    ...
    const handleSearch = (query: string) => {
        ...
        if (query.length === 13 && codeStart && query.startsWith(codeStart)) {
    ...
    // المنطق الرئيسي لإضافة منتج
    const handleAddProduct = (product: Product) => {
        if (!posSettings.config?.allowNegativeStock) {
            const currentQty = parseFloat(product.quantity as string) || 0;
        ...
        setCart(prev => {
            ...
            if (existing) {
                return prev.map(item =>
                    item.id === product.id ? { ...item, cartQuantity: item.cartQuantity + qty } : item
                );
            }
                ...
                <div className="flex-none bg-white px-4 pt-3 pb-0 flex gap-2 overflow-x-auto border-b border-gray-200 z-20">
                    <button
                        ...
                        className={cn(
                            ...
                            !selectedCategory ? "bg-[#0c4a3b] text-white border-[#083d2f]" : "bg-[#106b56] text-white/90 border-transparent hover:bg-[#0c4a3b]"
                        )}
                    >
                    ...
                    {categories.map(cat => (
                        <button
                            ...
                            className={cn(
                                "flex flex-col items-center justify-center py-2.5 px-4 min-w-[100px] rounded-t-xl transition-all border-b-4",
                                selectedCategory === cat ? "bg-[#0c4a3b] text-white border-[#083d2f]" : "bg-[#106b56] text-white/90 border-transparent hover:bg-[#0c4a3b]"
                            )}
                        >
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 h-full">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="font-bold text-gray-800 flex items-center gap-2">
                                ...
                                الأصناف المتاحة
                                <span className="text-sm text-gray-500 font-normal">({visibleProducts.length} صنف)</span>
                            </h2>
                            <button
                                    ...
                                    >
                                        ...
                                        <div className="w-16 h-16 flex items-center justify-center mt-2">
                                            <img
                                                ...
                                                onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/100x100/e2e8f0/64748b?text=No+Image' }}
                                            />
                                        </div>
                                        <div className="text-center w-full mt-auto">
                                            <div className="text-[12px] font-bold text-gray-800 leading-tight truncate">{product.name}</div>
                                        </div>
                                    </button>
                                );
                            })}
                ...
                <div className={cn(
                    ...
                    <div className="flex-none p-4 bg-white border-b border-gray-200 flex flex-col gap-3">
                        <div className="flex gap-2">
                            ...
                            <div className="flex items-center gap-1 border border-gray-300 rounded-lg px-1 bg-white">
                                <button onClick={() => setShowAddCustomerModal(true)} className="text-[#0c4a3b] hover:bg-green-50 p-2 rounded-md"><Plus size={16} /></button>
                                <button onClick={() => setShowCustomerDetailsModal(true)} className="text-[#0c4a3b] hover:bg-green-50 p-2 rounded-md"><Eye size={16} /></button>
                            </div>
                        </div>
                                ...
                                <div className="absolute z-[100] top-full mt-2 right-0 left-0 bg-white border-2 border-gray-200 rounded-xl shadow-2xl max-h-72 overflow-y-auto">
                                    {searchResults.map(product => (
                                        ...
                                        >
                                            ...
                                            <span className="text-[#0c4a3b] font-black bg-green-50 px-2 py-1 rounded-md">{product.price} ر.س</span>
                                        </button>
                                    ))}
                                </div>
                            )}
                                            ...
                                            <tr key={item.id} className="hover:bg-green-50/50 transition-colors">
                                                <td className="p-3 font-bold text-gray-800">
                                                    <div className="flex items-center gap-2">
                                                        {item.productNature === 'sub' && <Layers size={14} className="text-orange-500 shrink-0" />}
                                                        <span className="truncate">{item.name}</span>
                                                    </div>
                                                </td>
                                                <td className="p-3 text-center text-gray-600 font-bold">{price.toFixed(2)}</td>
                                                <td className="p-3 text-center">
                                                    <div className="flex items-center justify-center gap-1 bg-gray-100 rounded-lg px-1 py-1 mx-auto w-fit">
                                                        <button onClick={() => updateQuantity(item.id, -1)} className="w-6 h-6 flex items-center justify-center bg-white rounded-md shadow hover:bg-gray-50 font-bold text-gray-600">-</button>
                                                        <span className="font-black text-gray-800 w-8 text-center">{item.cartQuantity}</span>
                                                        <button onClick={() => updateQuantity(item.id, 1)} className="w-6 h-6 flex items-center justify-center bg-white rounded-md shadow hover:bg-gray-50 font-bold text-gray-600">+</button>
                                                    </div>
                                                </td>
                    ...
                    <div className="flex-none bg-gray-50 border-t-2 border-gray-200 p-4 z-30">
                        <div className="space-y-2 mb-3 text-sm">
                            ...
                            <div className="flex justify-between font-bold text-gray-600">
                                <span>ضريبة 15%:</span>
                                <span className="text-red-500">{(totalInvoice - (totalInvoice / 1.15)).toFixed(2)} ر.س</span>
                            </div>
                        ...
                        <div className="flex gap-2">
                            ...
                            <button
                                onClick={() => { setCart([]); setInvoiceDiscount(0); }}
        ...
        <main className="flex-1 flex flex-col overflow-hidden p-4 space-y-4">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-blue-100 p-4 grid grid-cols-1 md:grid-cols-3 gap-4 flex-none">
                <div className="space-y-2 relative" ref={searchRefBlue}>
                    <label className="text-sm font-bold text-blue-800 block">البحث عن صنف (اسم / باركود)</label>
                            ...
                            <div className="absolute z-[100] top-full mt-2 right-0 left-0 bg-white border-2 border-blue-200 rounded-xl shadow-2xl max-h-72 overflow-y-auto">
                                {searchResults.map(product => (
                                    ...
                                    >
                                        ...
                                        <span className="text-blue-700 font-black bg-blue-100 px-2 py-1 rounded-md">{product.price} ر.س</span>
                                    </button>
                                ))}
                            </div>
                        )}
                ...
                <div className="space-y-2 relative">
                    ...
                    <div className="flex relative">
                        ...
                        <input
                            ...
                            readOnly
                        />
                        <button onClick={() => setShowAddCustomerModal(true)} className="bg-blue-600 text-white px-4 hover:bg-blue-700 transition-colors flex items-center justify-center">
                            <Plus size={18} />
                        </button>
                        <button onClick={() => setShowCustomerDetailsModal(true)} className="bg-blue-500 text-white px-4 rounded-l-xl hover:bg-blue-600 transition-colors flex items-center justify-center border-l border-blue-600">
                            <Eye size={18} />
                        </button>
                    </div>
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex">
                        ...
                        />
                        <span className="bg-blue-100 text-blue-800 border-2 border-blue-200 px-4 py-2.5 rounded-l-xl flex items-center justify-center font-bold">مدين</span>
                    </div>
                </div>
            </div>
                        ...
                        <tbody className="divide-y divide-blue-50">
                            {cart.length === 0 ? (
                                ...
                                cart.map((item, index) => {
                                    ...
                                    return (
                                        <tr key={item.id} className={cn("hover:bg-blue-100/50 transition-colors", isEven ? "bg-white" : "bg-blue-50/30")}>
                                            <td className="p-3 text-center border-l border-blue-50">
                                                <button onClick={() => removeFromCart(item.id)} className="text-red-500 hover:bg-red-100 p-1.5 rounded-lg transition-colors inline-flex">
                                                    <Trash2 size={18} />
                                                </button>
                                            </td>
                                            <td className="p-3 text-center border-l border-blue-50 text-blue-800 font-medium">قطعة</td>
                                            <td className="p-3 text-center border-l border-blue-50 font-bold text-gray-700">{price.toFixed(2)}</td>
                                            <td className="p-3 text-center border-l border-blue-50">
                                                <div className="flex items-center justify-center gap-1">
                                                    <button onClick={() => updateQuantity(item.id, -1)} className="w-7 h-7 flex items-center justify-center bg-blue-100 text-blue-800 rounded hover:bg-blue-200 font-bold">-</button>
                                                    <input
                                                        type="number"
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            if (newQty > 0) {
                                                                setCart(prev => prev.map(cItem => cItem.id === item.id ? { ...cItem, cartQuantity: newQty } : cItem));
                                                            }
                                                        }}
                                            ...
                                            <td className="p-3 text-center border-l border-blue-50 text-blue-600">{(price * item.cartQuantity * 0.15).toFixed(2)}</td>
                                            <td className="p-3 text-center border-l border-blue-50 font-black text-blue-900 text-base">{(price * item.cartQuantity).toFixed(2)}</td>
                                            <td className="p-3 text-right font-bold text-gray-800">
                                                <div className="flex items-center gap-2 justify-end">
                                                    {item.name}
                                                    {item.productNature === 'sub' && <Layers size={14} className="text-orange-500" />}
                                                </div>
                                            </td>
                                        </tr>
                                    );
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-blue-100 p-4 flex-none">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100 flex flex-col items-center justify-center gap-1">
                        ...
                        <span className="text-xl font-black text-blue-900">{(totalInvoice / 1.15).toFixed(2)}</span>
                    </div>
                    <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100 flex flex-col items-center justify-center gap-1">
                        <span className="text-sm text-blue-600 font-bold">قيمة الضريبة (15%)</span>
                        <span className="text-xl font-black text-red-600">{(totalInvoice - (totalInvoice / 1.15)).toFixed(2)}</span>
                    </div>
                    <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100 flex flex-col items-center justify-center gap-1">
                ...
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-blue-100">
                    ...
                    <div className="flex items-center gap-4">
                        <label className="flex items-center gap-2 cursor-pointer text-blue-800 font-bold select-none">
                            <input type="checkbox" className="w-5 h-5 accent-blue-600 rounded border-gray-300" defaultChecked />
                            <span>طباعة تلقائية</span>
                        </label>
                        <div className="h-8 w-px bg-blue-200"></div>
                        <button className="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center text-blue-600 hover:bg-blue-100 border border-blue-100 transition-all" title="طباعة">
                            <Printer size={24} />
                        </button>
                    </div>
                </div>
    ...
    return (
        <div className="w-full h-full relative flex flex-col bg-[#eef1f5] overflow-hidden" dir={direction || 'rtl'}>
            {/* Header الديناميكي (يتغير لونه حسب الشاشة) */}
            <header className={cn(
                ...
                <div className="flex items-center gap-4">
                    <div className={cn(
                        ...
                        isBlueScreen ? "bg-blue-600/50 text-white border-blue-500/50" : "bg-[#0c4a3b]/10 text-[#0c4a3b] border-[#0c4a3b]/20"
                    )}>
                        <Building size={16} />
                        <span className="text-sm font-bold">{selectedBranch?.name || 'الفرع الرئيسي'}</span>
                    </div>
                    {isBlueScreen && (
                ...
                <div className="flex items-center gap-6">
                    <div className={cn(
                        ...
                        isBlueScreen ? "bg-blue-800/50 text-white border-blue-600" : "text-gray-600 bg-gray-50 border-gray-200"
                    )}>
                        <Clock size={16} className={isBlueScreen ? "text-blue-200" : "text-[#0c4a3b]"} />
                        <span>{currentTime.toLocaleTimeString('en-GB')}</span>
                        <span className="mx-1 opacity-50">|</span>
                        <Calendar size={16} className={isBlueScreen ? "text-blue-200" : "text-[#0c4a3b]"} />
                    ...
                    <div className={cn(
                        ...
                        <div className="text-right">
                            ...
                            <div className={cn("text-[9px] font-bold", isBlueScreen ? "text-blue-600" : "text-[#00a651]")}>متصل</div>
                        </div>
                    </div>
                </div>
            </header>

            {/* محتوى الشاشة بناءً على إعدادات الفرع */}
            {selectedBranch?.showScreen2 && !selectedBranch?.showTouchScreen ? (
                ...
                /* إذا لم يتم تحديد شاشة أو تم تحديد الاثنين معاً (يفضل جعل إحداهما افتراضية في حال الخطأ) */
                selectedBranch?.showTouchScreen ? renderTouchScreen() : renderTraditionalScreen()
            )}

            {/* ===================================================================== */}
            {/* نافذة الأصناف المتفرعة (نفسها لكلا الشاشتين) */}
            {/* ===================================================================== */}
            {showSubProductsModal && currentParentProduct && (
                <div
                    ...
                    onClick={() => { setShowSubProductsModal(false); setCurrentParentProduct(null); }}
                >
                    <div
                    ...
                    >
                        ...
                        <div className="p-4 space-y-2 max-h-80 overflow-y-auto bg-gray-50">
                            {subProducts
                                .filter(sub => sub.parentProductId === currentParentProduct.id || sub.parentProductName === currentParentProduct.name)
                                .map(sub => {
                                    const price = parseFloat(String(sub.price).replace(/[^0-9.]/g, '')) || 0;
                                    return (
                                        <button
                                            key={sub.id}
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <span className="font-bold text-gray-800 text-base block">{sub.name}</span>
                                                    {sub.category && <span className="text-xs text-gray-500">{sub.category}</span>}
                                                </div>
                                            </div>
                                            <span className="font-black text-orange-600 text-xl">{price.toFixed(2)}</span>
                                        </button>
                                    );
                                })}
                        </div>
            ...
            {/* نوافذ العملاء والإيصال */}
            {showCustomerDetailsModal && (
                <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowCustomerDetailsModal(false)}>
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div className={cn("p-4 flex justify-between items-center text-white", isBlueScreen ? "bg-blue-600" : "bg-[#0c4a3b]")}>
                            <h2 className="font-bold text-lg flex items-center gap-2"><User size={20} /> بيانات العميل</h2>
                            <button onClick={() => setShowCustomerDetailsModal(false)} className="text-white/80 hover:text-white"><X size={20} /></button>
                        </div>
                        <div className="p-4 space-y-3 bg-white">
                            <div className="flex justify-between py-3 border-b border-gray-100"><span className="font-bold text-gray-500">الاسم:</span><span className="font-bold text-gray-800">{customer.name}</span></div>
                            <div className="flex justify-between py-3 border-b border-gray-100"><span className="font-bold text-gray-500">الهاتف:</span><span className="font-bold text-gray-800">{customer.phone}</span></div>
                            <div className="flex justify-between py-3 border-b border-gray-100"><span className="font-bold text-gray-500">المجموعة:</span><span className="font-bold text-gray-800">{customer.group}</span></div>
                        </div>
                        <div className="p-4 border-t border-gray-100 flex gap-2 justify-end bg-gray-50">
                            <button onClick={() => setShowCustomerDetailsModal(false)} className="px-6 py-2 bg-gray-200 rounded-lg font-bold text-gray-700 hover:bg-gray-300 transition-colors">إغلاق</button>
                            <button onClick={() => { setShowCustomerDetailsModal(false); setShowEditCustomerModal(true); }} className={cn("px-6 py-2 text-white rounded-lg font-bold transition-colors", isBlueScreen ? "bg-blue-600 hover:bg-blue-700" : "bg-[#0c4a3b] hover:bg-[#0a3d32]")}>تعديل</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {showEditCustomerModal && (
                <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowEditCustomerModal(false)}>
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div className={cn("p-4 flex justify-between items-center text-white", isBlueScreen ? "bg-blue-600" : "bg-[#0c4a3b]")}>
                            <h2 className="font-bold text-lg flex items-center gap-2"><Pencil size={20} /> تعديل العميل</h2>
                            <button onClick={() => setShowEditCustomerModal(false)} className="text-white/80 hover:text-white"><X size={20} /></button>
                        </div>
                        <div className="p-6 space-y-5 bg-white">
                            <div>
                                <label className={cn("block text-sm font-bold mb-2", isBlueScreen ? "text-blue-800" : "text-[#0c4a3b]")}>الاسم *</label>
                                <input type="text" value={editCustomerData.name} onChange={(e) => setEditCustomerData({ ...editCustomerData, name: e.target.value })} className={cn("w-full border-2 rounded-lg px-4 py-2.5 outline-none font-bold", isBlueScreen ? "border-blue-100 focus:border-blue-500 bg-blue-50/30" : "border-gray-200 focus:border-[#0c4a3b] bg-gray-50")} />
                            </div>
                            <div>
                                <label className={cn("block text-sm font-bold mb-2", isBlueScreen ? "text-blue-800" : "text-[#0c4a3b]")}>الهاتف</label>
                                <input type="text" value={editCustomerData.phone} onChange={(e) => setEditCustomerData({ ...editCustomerData, phone: e.target.value })} className={cn("w-full border-2 rounded-lg px-4 py-2.5 outline-none font-bold", isBlueScreen ? "border-blue-100 focus:border-blue-500 bg-blue-50/30" : "border-gray-200 focus:border-[#0c4a3b] bg-gray-50")} />
                            </div>
                        </div>
                        <div className="p-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-2">
                            <button onClick={() => setShowEditCustomerModal(false)} className="px-6 py-2.5 bg-gray-200 text-gray-700 rounded-lg font-bold hover:bg-gray-300 transition-colors">إلغاء</button>
                            <button onClick={handleEditCustomerSave} className={cn("px-8 py-2.5 text-white rounded-lg font-bold transition-colors", isBlueScreen ? "bg-blue-600 hover:bg-blue-700" : "bg-[#0c4a3b] hover:bg-[#0a3d32]")}>حفظ التعديلات</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {showAddCustomerModal && (
                <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowAddCustomerModal(false)}>
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div className={cn("p-4 flex justify-between items-center text-white", isBlueScreen ? "bg-blue-600" : "bg-[#0c4a3b]")}>
                            <h2 className="font-bold text-lg flex items-center gap-2"><Plus size={20} /> إضافة عميل جديد</h2>
                            <button onClick={() => setShowAddCustomerModal(false)} className="text-white/80 hover:text-white"><X size={20} /></button>
                        </div>
                        <div className="p-6 space-y-5 bg-white">
                            <div>
                                <label className={cn("block text-sm font-bold mb-2", isBlueScreen ? "text-blue-800" : "text-[#0c4a3b]")}>الاسم *</label>
                                <input type="text" className={cn("w-full border-2 rounded-lg px-4 py-2.5 outline-none font-bold", isBlueScreen ? "border-blue-100 focus:border-blue-500 bg-blue-50/30" : "border-gray-200 focus:border-[#0c4a3b] bg-gray-50")} placeholder="أدخل اسم العميل" />
                            </div>
                            <div>
                                <label className={cn("block text-sm font-bold mb-2", isBlueScreen ? "text-blue-800" : "text-[#0c4a3b]")}>الهاتف</label>
                                <input type="text" className={cn("w-full border-2 rounded-lg px-4 py-2.5 outline-none font-bold", isBlueScreen ? "border-blue-100 focus:border-blue-500 bg-blue-50/30" : "border-gray-200 focus:border-[#0c4a3b] bg-gray-50")} placeholder="رقم الجوال" />
                            </div>
                        </div>
                        <div className="p-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-2">
                            <button onClick={() => setShowAddCustomerModal(false)} className="px-6 py-2.5 bg-gray-200 text-gray-700 rounded-lg font-bold hover:bg-gray-300 transition-colors">إلغاء</button>
                            <button onClick={() => setShowAddCustomerModal(false)} className={cn("px-8 py-2.5 text-white rounded-lg font-bold transition-colors", isBlueScreen ? "bg-blue-600 hover:bg-blue-700" : "bg-[#0c4a3b] hover:bg-[#0a3d32]")}>إضافة العميل</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {/* Receipt Modal */}
            {showReceiptModal && (
                <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowReceiptModal(false)}>
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                        <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-100 text-gray-700">
                            <h2 className="font-bold text-lg flex items-center gap-2"><Printer size={20} /> الإيصال</h2>
                        ...
                        <div className="p-8 overflow-y-auto flex-1 font-mono text-sm bg-white">
                            <div className="text-center mb-6 font-bold text-lg">{posSettings?.receipt?.header || 'اسم المتجر'}</div>
                                    ...
                                    {cart.map(item => {
                                        ...
                                        return (
                                            <tr key={item.id}>
                                                ...
                                                <td className="py-2 text-left font-semibold">{(price * item.cartQuantity).toFixed(2)}</td>
                                            </tr>
                                        );
                                    })}
                                </tbody>
                            ...
                            <div className="border-t-2 border-dashed border-gray-300 my-4" />
                            <div className="text-center mt-6 text-gray-500 font-bold">{posSettings?.receipt?.footer || 'شكراً لزيارتكم!'}</div>
                        </div>
                        <div className="p-4 bg-gray-100 border-t border-gray-200 flex gap-3">
                            <button onClick={() => { setCart([]); setShowReceiptModal(false); setInvoiceDiscount(0); }} className={cn("flex-1 text-white py-3 rounded-xl font-bold transition-all shadow-md text-base", isBlueScreen ? "bg-blue-600 hover:bg-blue-700" : "bg-[#0c4a3b] hover:bg-[#0a3d32]")}>بيع جديد</button>
                            <button onClick={() => window.print()} className="bg-white text-gray-800 px-6 py-3 rounded-xl font-bold hover:bg-gray-50 flex items-center gap-2 border border-gray-300 shadow-sm transition-all"><Printer size={20} /> طباعة</button>
                        </div>
                    </div>
                </div>
            )}
