...
const Warehouses: React.FC = () => {
  ...
  const filteredWarehouses = warehouses.filter((w) => w.name.toLowerCase().includes(searchTerm.toLowerCase()) || w.code.toLowerCase().includes(searchTerm.toLowerCase()) || w.address.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="p-6 space-y-6" dir={direction}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[var(--bg-card)] rounded-xl border border-[var(--border)] overflow-hidden shadow-sm">
                    ...
                    <tr key={`desktop-${w.id}`} className="border-b border-[var(--border)] hover:bg-[var(--bg-main)]/50 transition-colors">
                      <td className="p-3">
                        <div className="flex items-center gap-2 justify-center">
                          <button onClick={() => handleOpenModal(w)} className="p-1.5 text-[var(--primary)] hover:bg-[var(--primary)]/10 rounded-md transition-colors">
                            <Edit2 size={16} />
                          </button>
                          <button onClick={() => deleteWarehouse(w.id)} className="p-1.5 text-[var(--primary)] hover:bg-[var(--primary)]/10 rounded-md transition-colors">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
      ...
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm overflow-y-auto">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl my-8">
              <div className="bg-primary p-4 flex items-center justify-between text-white">
                ...
                <span className="font-bold text-lg">{editingWarehouse ? t("edit_warehouse") : t("add_warehouse")}</span>
              </div>
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                <div className="text-sm text-[var(--primary)] text-right leading-relaxed font-bold">برجاء ادخال المعلومات أدناه. تسميات الحقول التي تحمل علامة * هي حقول اجبارية .</div>
                ...
                <div className="space-y-4 text-right">
                  ...
                  <div>
                    ...
                    <textarea required value={form.address || ""} onChange={(e) => setForm({ ...form, address: e.target.value })} rows={4} className="w-full p-3 border border-gray-300 rounded-xl bg-white text-gray-900 focus:ring-2 focus:ring-[var(--primary)]/10 outline-none text-right transition-all resize-none" />
                  </div>
                </div>

                <div className="flex justify-end pt-2">
                  <button type="submit" className="bg-[var(--primary)] text-white px-8 py-3 rounded-xl hover:bg-[var(--primary-hover)] transition-all font-bold shadow-lg shadow-[var(--primary)]/10 active:scale-95">
