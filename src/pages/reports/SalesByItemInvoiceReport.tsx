...
import { FileText, FileSpreadsheet, Printer, Search, Grid, Share2, ChevronDown, ChevronUp } from 'lucide-react';
import { useLanguage } from '@/context/LanguageContext';
import { cn } from '@/lib/utils';

export default function SalesByItemInvoiceReport() {
    ...
    <div className="p-4 space-y-6 bg-white dark:bg-gray-900 min-h-screen" dir={direction}>
      ...
      <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold text-[var(--primary)]">تقرير مبيعات الأصناف حسب الفاتورة</h1>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-1 text-right">
            <label className="text-sm font-bold text-[var(--primary)]">كاشير</label>
          ...
          <div className="space-y-1 text-right">
            <label className="text-sm font-bold text-[var(--primary)]">صنف</label>
            <input type="text" className="w-full border border-gray-300 dark:border-gray-600 rounded px-3 py-2 text-sm outline-none focus:border-[var(--primary)] dark:bg-gray-700 text-right" />
          </div>
          <div className="flex items-end">
            <button className="w-full bg-[var(--primary)] text-white px-6 py-2 rounded text-sm font-bold hover:bg-[var(--primary-hover)] transition-colors">اتمام العملية</button>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-[10px] text-right border-collapse">
            <thead>
              <tr className="bg-[var(--primary)] text-white">
                ...
                <th className="px-1 py-2 border border-[var(--primary-hover)] font-bold">كود الصنف</th>
                <th className="px-1 py-2 border border-[var(--primary-hover)] font-bold">اسم الصنف</th>
                <th className="px-1 py-2 border border-[var(--primary-hover)] font-bold">كمية</th>
                <th className="px-1 py-2 border border-[var(--primary-hover)] font-bold">السعر</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td colSpan={7} className="px-3 py-4 text-center font-bold text-gray-500">لا توجد بيانات في الجدول</td>
              </tr>
            </tbody>
            <tfoot className="bg-gray-50 dark:bg-gray-800/50 font-bold">
              <tr>
                <td className="px-1 py-2 border border-gray-200 dark:border-gray-700 text-gray-400">[كود الصنف]</td>
                <td colSpan={3} className="px-1 py-2 border border-gray-200 dark:border-gray-700"></td>
                <td className="px-1 py-2 border border-gray-200 dark:border-gray-700 text-gray-400">[اسم الصنف]</td>
                <td className="px-1 py-2 border border-gray-200 dark:border-gray-700">0.00</td>
                <td className="px-1 py-2 border border-gray-200 dark:border-gray-700">السعر 0.00</td>
              </tr>
            </tfoot>
          </table>
        </div>
