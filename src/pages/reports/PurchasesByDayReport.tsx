...
const mockRows = [
  { id: "1", date: "2026-07-01", purchases: 1850.46, tax: 277.54, totalWithTax: 2128.0 },
  { id: "2", date: "2026-07-02", purchases: 1916.75, tax: 287.25, totalWithTax: 2204.0 },
  { id: "3", date: "2026-07-03", purchases: 1835.77, tax: 275.23, totalWithTax: 2111.0 },
  { id: "4", date: "2026-07-04", purchases: 1085.25, tax: 162.75, totalWithTax: 1248.0 },
  { id: "5", date: "2026-07-05", purchases: 2611.45, tax: 391.55, totalWithTax: 3003.0 },
];
        ...
        <CardHeader className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              {t("purchases_by_day", "تحليل مشتريات على مستوى الأيام")}
            </CardTitle>
          ...
          <div className="rounded-2xl border border-gray-100 dark:border-slate-800 bg-white dark:bg-transparent p-4 md:p-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
              <div className="space-y-2 lg:col-span-1">
                ...
                <ComboboxField value={filters.fiscalYear} onChange={handleYearChange} items={FISCAL_YEARS.map((y) => ({ id: y, name: y }))} valueKey="id" labelKey="name" placeholder={t("select_year", "اختر السنة")} />
              </div>
              <div className="space-y-2 lg:col-span-1">
                <label className="text-xs font-medium text-[var(--text-main)]">{t("fiscal_quarter", "الربع المالي")}</label>
                <ComboboxField value={filters.fiscalQuarter} onChange={handleQuarterChange} items={FISCAL_QUARTERS.map((q) => ({ id: q.value, name: q.label }))} valueKey="id" labelKey="name" placeholder={t("select_quarter", "اختر الربع")} />
              </div>
              <div className="space-y-2 lg:col-span-1">
                <label className="text-xs font-medium text-[var(--text-main)]">{t("from_date", "تاريخ البداية")}</label>
                <input type="date" value={filters.from} onChange={(e) => setFilters((p) => ({ ...p, from: e.target.value, fiscalQuarter: "" }))} className="takamol-input h-10 w-full" />
              </div>
              <div className="space-y-2 lg:col-span-1">
                <label className="text-xs font-medium text-[var(--text-main)]">{t("to_date", "تاريخ النهاية")}</label>
                <input type="date" value={filters.to} onChange={(e) => setFilters((p) => ({ ...p, to: e.target.value, fiscalQuarter: "" }))} className="takamol-input h-10 w-full" />
              </div>
