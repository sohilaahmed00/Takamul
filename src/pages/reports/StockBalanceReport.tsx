...
import { PackageSearch, RotateCcw, Search, Printer, FileText, FileSpreadsheet } from "lucide-react";
...
export default function StockBalanceReport() {
  ...
  const totalCostValue = useMemo(() => reportData?.reduce((s, r) => s + r.totalCostValue, 0) ?? 0, [reportData]);
  const totalSaleValue = useMemo(() => reportData?.reduce((s, r) => s + r.totalSaleValue, 0) ?? 0, [reportData]);
        ...
        <CardContent className="space-y-5">
          ...
          <div className="hidden xl:block rounded-xl border border-gray-100 dark:border-slate-800 overflow-hidden">
            <DataTable
              ...
              onPage={(e: DataTablePageEvent) => {
                if (e.page !== undefined) setCurrentPage(e.page + 1);
              }}
              dataKey="productId"
          ...
          <div className="grid grid-cols-1 gap-5 xl:hidden">
            {isLoading || isFetching ? (
              ...
              <div className="rounded-2xl border border-dashed border-gray-200 dark:border-slate-800 bg-[#fafafa] dark:bg-slate-900/20 p-8 text-center text-sm text-[var(--text-muted)]">{t("no_data", "لا توجد بيانات")}</div>
            ) : (
              reportData.slice((currentPage - 1) * entriesPerPage, currentPage * entriesPerPage).map((row) => (
                <div key={row.productId} className="rounded-2xl border border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900/40 shadow-sm overflow-hidden">
                  <div className="flex items-center justify-between gap-3 px-4 py-3 bg-[#f8fafc] dark:bg-slate-900/60 border-b border-gray-100 dark:border-slate-800">
                    ...
                    <div className="shrink-0 rounded-full px-3 py-1 text-xs font-medium bg-[rgba(49,201,110,0.12)] text-[var(--primary)]">{row.unitName}</div>
                  </div>
            ...
            {reportData && reportData.length > entriesPerPage && (
              <div className="flex items-center justify-center gap-2 pt-2">
                <button type="button" onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))} disabled={currentPage === 1} className="h-10 px-4 rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 text-sm disabled:opacity-50">
                  {t("previous", "السابق")}
                </button>
                ...
                <button type="button" onClick={() => setCurrentPage((p) => (p < Math.ceil((reportData?.length ?? 0) / entriesPerPage) ? p + 1 : p))} disabled={currentPage >= Math.ceil((reportData?.length ?? 0) / entriesPerPage)} className="h-10 px-4 rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 text-sm disabled:opacity-50">
                  {t("next", "التالي")}
                </button>
              </div>
            )}
