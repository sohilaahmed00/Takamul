...
const InventoryTransfersReport = () => {
    ...
    <div className="p-6 bg-white min-h-screen" dir={dir}>
      ...
      <div className="bg-white rounded-t-lg border-b border-gray-200 p-4 flex justify-between items-center">
        <div className="flex items-center gap-2 text-[var(--primary)]">
          ...
          <h1 className="text-xl font-bold">تقرير تحويلات المخزون</h1>
        </div>
          ...
          <table className="w-full text-right border-collapse">
            <thead>
              <tr className="bg-[var(--primary)] text-white">
                ...
                <th className="p-3 border border-[var(--primary-hover)]">من الفرع</th>
                <th className="p-3 border border-[var(--primary-hover)]">الي الفرع</th>
            ...
            <tbody>
              {transfers?.length > 0 ? (
                transfers?.map((transfer, index) => (
                  <tr key={transfer.id} className={cn(index % 2 === 0 ? 'bg-white' : 'bg-[var(--primary)]/5')}>
                    <td className="p-3 border border-gray-200 text-sm">{transfer.date}</td>
                    <td className="p-3 border border-gray-200 text-sm">{transfer.reference}</td>
                    <td className="p-3 border border-gray-200 text-sm">{transfer.fromBranch}</td>
                    <td className="p-3 border border-gray-200 text-sm">{transfer.toBranch}</td>
                    <td className="p-3 border border-gray-200 text-sm">{transfer.user}</td>
                    <td className="p-3 border border-gray-200 text-sm">{transfer.items}</td>
                    <td className="p-3 border border-gray-200 text-sm font-bold">{transfer.quantity}</td>
                  </tr>
                ))
              ) : (
                <tr>
              ...
              <tr className="bg-gray-50 font-bold text-gray-600">
                ...
                <td className="p-3 border border-gray-200 text-xs">[من الفرع]</td>
                <td className="p-3 border border-gray-200 text-xs">[الي الفرع]</td>
                ...
                <td className="p-3 border border-gray-200 text-sm">كمية</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default InventoryTransfersReport;
