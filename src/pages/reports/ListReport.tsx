...
const ListReport = () => {
  ...
  const listItems = [
    { id: 1, code: '21212121212121', name: 'مياه', quantity: 1.00, amount: 50.00 }
  ];
    ...
    <div className="p-6 bg-white min-h-screen" dir={dir}>
      ...
      <div className="bg-white rounded-t-lg border-b border-gray-200 p-4 flex justify-between items-center">
        <div className="flex items-center gap-2 text-[var(--primary)]">
          <Heart className="w-5 h-5 fill-[var(--primary)]" />
          <h1 className="text-xl font-bold">تقرير الليسته</h1>
        </div>
          ...
          <table className="w-full text-right border-collapse">
            <thead>
              <tr className="bg-[var(--primary)] text-white">
                <th className="p-3 border border-[var(--primary-hover)]">كود الصنف</th>
                <th className="p-3 border border-[var(--primary-hover)]">اسم الصنف</th>
                <th className="p-3 border border-[var(--primary-hover)]">إجمالي الكمية</th>
                <th className="p-3 border border-[var(--primary-hover)]">إجمالي مبلغ الليسته</th>
              </tr>
            </thead>
            <tbody>
              {listItems.map((item, index) => (
                <tr key={item.id} className={cn(index % 2 === 0 ? 'bg-white' : 'bg-[var(--primary)]/5')}>
                  <td className="p-3 border border-gray-200 text-sm">{item.code}</td>
                  <td className="p-3 border border-gray-200 text-sm">{item.name}</td>
                  <td className="p-3 border border-gray-200 text-sm font-bold">{item.quantity.toFixed(2)}</td>
                  <td className="p-3 border border-gray-200 text-sm font-bold">{item.amount.toFixed(2)}</td>
                </tr>
              ))}
              {/* Footer Row */}
              <tr className="bg-gray-100 font-bold text-gray-600">
                ...
                <td className="p-3 border border-gray-200 text-sm">1.00</td>
                <td className="p-3 border border-gray-200 text-sm">50.00</td>
              </tr>
            </tbody>
          </table>
        </div>
