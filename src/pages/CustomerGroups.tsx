...
import { Plus, Search, Edit2, Trash2, Users, ChevronLeft, ChevronRight, MoreVertical, Link as LinkIcon } from "lucide-react";
import { useLanguage } from "@/context/LanguageContext";
import { useCustomerGroups, type CustomerGroup,  } from "@/context/CustomerGroupsContext";
...
const CustomerGroups = () => {
  ...
  return (
    <div className="space-y-6" dir={direction}>
      ...
      <div className="takamol-card overflow-hidden">
        ...
        <div className="p-4 border-b border-[var(--border)] flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder={t("search_placeholder")} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full md:w-64 pr-10 pl-4 py-2 bg-[var(--bg-card)] border border-[var(--border)] text-[var(--text-main)] rounded-xl text-sm outline-none focus:ring-2 focus:ring-[var(--primary)] transition-all" />
          </div>
        </div>
            ...
            <tbody>
              <AnimatePresence mode="popLayout">
                {currentGroups.map((group) => (
                  <motion.tr key={`desktop-${group.id}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <td className="px-6 py-4 text-sm font-medium text-[var(--text-main)] border-x border-[var(--border)]">{group.name}</td>
                    <td className="px-6 py-4 text-sm text-[var(--text-muted)] border-x border-[var(--border)]">{group.percentage}</td>
                    <td className="px-6 py-4 border-x border-[var(--border)]">
                      <div className="flex items-center justify-center gap-2">
                        <button onClick={() => handleEdit(group)} className="p-2 text-[var(--primary)] hover:bg-[var(--primary)]/10 rounded-lg transition-colors" title={t("edit")}>
                          <Edit2 size={18} />
                        </button>
                        <button onClick={() => deleteGroup(group.id)} className="p-2 text-[var(--primary)] hover:bg-[var(--primary)]/10 rounded-lg transition-colors" title={t("delete")}>
                          <Trash2 size={18} />
                        </button>
                      </div>
                    </td>
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-[var(--text-muted)]">
                    {t("percentage")}: {group.percentage}
                  </p>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => handleEdit(group)} className="p-2 text-[var(--primary)] bg-[var(--bg-card)] rounded-lg shadow-sm">
                    <Edit2 size={18} />
                  </button>
                  <button onClick={() => deleteGroup(group.id)} className="p-2 text-[var(--primary)] bg-[var(--bg-card)] rounded-lg shadow-sm">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
        ...
        <div className="p-4 border-t border-[var(--border)] flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-2 rounded-lg border border-[var(--border)] disabled:opacity-50 hover:bg-[var(--table-row-hover)] transition-colors text-[var(--text-main)]">
              <ChevronRight size={18} />
            </button>
            <div className="flex items-center gap-1">
              {[...Array(totalPages)].map((_, i) => (
                <button key={i} onClick={() => setCurrentPage(i + 1)} className={cn("w-8 h-8 rounded-lg text-sm font-medium transition-all", currentPage === i + 1 ? "bg-[var(--primary)] text-white shadow-md" : "hover:bg-[var(--table-row-hover)] text-[var(--text-muted)]")}>
                  {i + 1}
                </button>
              ))}
            </div>
            <button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-2 rounded-lg border border-[var(--border)] disabled:opacity-50 hover:bg-[var(--table-row-hover)] transition-colors text-[var(--text-main)]">
              <ChevronLeft size={18} />
            </button>
          </div>
        </div>
      ...
      <CustomerGroupsModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleSave} initialData={editingGroup} />
    </div>
  );
};
