...
import {
  useCustomerGroups,
  type CustomerGroup,
} from "@/context/CustomerGroupsContext";
      ...
      <div className="takamol-card overflow-hidden">
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="takamol-table w-full text-right">
            <thead>
              <tr>
                <th className="px-6 py-4 font-bold text-sm border border-[var(--table-header-border)]">
                  {t("name")}
                </th>
                <th className="px-6 py-4 font-bold text-sm border border-[var(--table-header-border)]">
                  {t("percentage")}
                </th>
                <th className="px-6 py-4 font-bold text-sm border border-[var(--table-header-border)] text-center">
                  {t("actions")}
                </th>
              </tr>
            </thead>
        ...
        <div className="md:hidden p-4 space-y-4">
          {currentGroups.map((group) => (
            <div
              key={`mobile-${group.id}`}
              className="bg-[var(--bg-main)] p-4 rounded-xl border border-[var(--border)] space-y-3"
            >
        ...
        <div className="p-4 border-t border-[var(--border)] flex flex-col md:flex-row md:items-center justify-between gap-4">
          <p className="text-sm text-[var(--text-muted)]">
            {t("showing_records")} {filteredGroups.length === 0 ? 0 : startIndex + 1}{" "}
            {t("to")} {Math.min(startIndex + itemsPerPage, filteredGroups.length)}{" "}
            {t("of")} {filteredGroups.length} {t("records")}
          </p>
          <div className="flex items-center gap-2">
            <button
