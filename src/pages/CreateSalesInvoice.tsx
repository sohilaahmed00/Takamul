...
const CreateSalesInvoice: React.FC = () => {
  ...
  const { data: salesOrder } = useGetSalesOrderById(
  ...
  useEffect(() => {
    ...
    const customer = customers.find(
      (c) => c.customerName === salesOrder.customerName
    );
    const warehouse = wareHouses.find(
      (w) => w.warehouseName === salesOrder.warehouseName
    );
    ...
    form.reset({
          ...
          ? salesOrder.payments.map((p) => ({
              ...
              paymentMethod:
                p.paymentMethod === "Visa"
                  ...
                  ? "BankTransfer"
                  : (p.paymentMethod as SalesInvoiceType["payments"][number]["paymentMethod"]),
            }))
          : [
              {
                amount: 0,
      ...
      invoiceDiscountType:
        (salesOrder as any).globalDiscountValue > 0 ? "fixed" : "percentage",
      invoiceDiscountValue:
        (salesOrder as any).globalDiscountValue > 0
          ? (salesOrder as any).globalDiscountValue
          : ((salesOrder as any).globalDiscountPercentage ?? 0),
    });
  ...
  const handleSubmit = async (data: SalesInvoiceType) => {
    const payload: CreateSalesOrder = {
      ...
      globalDiscountPercentage:
        data.invoiceDiscountType === "percentage"
          ? (data.invoiceDiscountValue ?? 0)
          : 0,
      globalDiscountValue:
        data.invoiceDiscountType === "fixed"
          ? (data.invoiceDiscountValue ?? 0)
          : 0,
  ...
  return (
    <Card dir={direction}>
      <CardHeader>
        <CardTitle>
          {isEditMode
            ? `${t("edit_sales_invoice")} #${salesOrder?.orderNumber ?? id}`
            : t("add_sales_invoice")}
        </CardTitle>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
              <div className="lg:col-span-2 space-y-4">
                {paymentFields.map((payment, index) => (
                  <div
                    key={payment.id}
                    className="flex flex-col sm:flex-row gap-3 items-center bg-gray-50/50 p-3 rounded-lg border border-gray-100"
                  >
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <Input
                            ...
                            className="text-center bg-white flex-1"
                            placeholder={t("value")}
                          />
                        )}
                      />
