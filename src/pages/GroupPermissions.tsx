...
import { useGroups, ModulePermissions, defaultPermissions } from '@/context/GroupsContext';
...
const GroupPermissions = () => {
  ...
  const handleToggleModule = (module: string) => {
    ...
    const allChecked = perm.view && perm.add && perm.edit && perm.delete && 
      (!perm.extra || Object.values(perm.extra).every(v => v));
  ...
  const handleToggleAll = () => {
    ...
    const allChecked = permsList.every(p => 
      p.view && p.add && p.edit && p.delete && (!p.extra || Object.values(p.extra).every(v => v))
    );
    ...
    return (
      <div className="select-none">
        ...
        >
          ...
          >
            {moduleId ? (
              isExpanded ? <ChevronDown size={14} /> : (direction === 'rtl' ? <ChevronLeft size={14} /> : <ChevronRight size={14} />)
            ) : null}
          </div>
          ...
          <span className={`text-sm ${level === 0 ? 'font-bold' : 'font-medium'} text-gray-700`}>
            {label}
          </span>
        </div>
  ...
  const permsList = Object.values(permissions) as ModulePermissions[];
  const allChecked = permsList.length > 0 && permsList.every(p => 
    p.view && p.add && p.edit && p.delete && (!p.extra || Object.values(p.extra).every(v => v))
  );
  ...
  return (
    <div className="space-y-4 max-w-4xl mx-auto" dir={direction}>
      <div className={`flex justify-between items-center px-2`}>
        <div className="text-[var(--primary)] font-bold text-xs">
          {language === 'ar' ? 'الرجاء تعيين الصلاحيات للمجموعة ادناه' : 'Please set permissions for the group below'}
        </div>
        <button 
          onClick={handleToggleAll}
          className="flex items-center gap-2 text-xs font-bold text-[var(--primary)] hover:opacity-80 transition-opacity"
        >
      ...
      <div className="bg-[var(--table-header)] p-2.5 text-[var(--table-header-text)] flex justify-center items-center font-bold text-xs shadow-sm rounded-t-lg">
        <span>
          {language === 'ar' 
            ? `${group.description} ( ${group.name} ) ضوابط وصلاحيات المجموعة` 
            : `Group Permissions: ${group.description} (${group.name})`}
        </span>
      </div>
      ...
      <div className="bg-white border border-gray-200 shadow-sm overflow-hidden rounded-b-lg p-4">
        <div className="space-y-1">
          {modules.map((module) => {
            ...
            const moduleChecked = perm.view && perm.add && perm.edit && perm.delete && 
              (!perm.extra || Object.values(perm.extra).every(v => v));
                ...
                <div className="space-y-0.5">
                  {(['view', 'add', 'edit', 'delete'] as const).map(field => (
                    <PermissionTree
                      ...
                      checked={perm[field]}
                      onToggle={() => handleToggleField(module, field)}
                    />
                  ))}
                  ...
                  {perm.extra && (
                    <PermissionTree
                      ...
                      checked={Object.values(perm.extra).every(v => v)}
                      onToggle={() => handleToggleField(module, 'extra')}
                    >
                      <div className="space-y-0.5">
                        {Object.keys(perm.extra).map(extraKey => (
                          <PermissionTree
                            key={`${module}-extra-${extraKey}`}
                            level={2}
                            label={getLabel(extraKey)}
                            checked={perm.extra![extraKey]}
                            onToggle={() => handleToggleExtra(module, extraKey)}
                          />
                        ))}
                      </div>
                    </PermissionTree>
