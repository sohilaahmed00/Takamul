...
export default function ReturnPOSSale() {
  ...
  const [payments, setPayments] = useState([{ id: Date.now(), ref: "", amount: "0", method: "شبكة", notes: "" }]);
  ...
  const addPayment = () => {
    setPayments((prev) => [...prev, { id: Date.now(), ref: "", amount: "0", method: "شبكة", notes: "" }]);
  };
  ...
  return (
    <div className="space-y-6 pb-12" dir={direction}>
      ...
      <div className="bg-primary text-white p-2 rounded-md text-center text-sm font-medium flex items-center justify-center gap-2 cursor-pointer hover:bg-primary-hover transition-colors">
        <span>
          {t("subscription_expires_in")} 933 {t("days")}
        </span>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="p-6 space-y-8">
          ...
          <div className="space-y-4">
            {payments.map((payment, index) => (
              <div key={payment.id} className="relative grid grid-cols-1 md:grid-cols-3 gap-6 text-right border border-gray-200 p-6 rounded-xl bg-gray-50/30">
                {index > 0 && (
                  <button onClick={() => removePayment(payment.id)} className={cn("absolute top-2 text-[var(--primary)] hover:text-[var(--primary-hover)]", direction === "rtl" ? "left-2" : "right-2")}>
                    <X size={16} />
                  </button>
                )}
                <div className="space-y-1">
          ...
          <button onClick={addPayment} className="w-full bg-primary text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2 hover:bg-primary-hover transition-colors">
      ...
      <AnimatePresence>
        {showItemsModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4" onClick={() => setShowItemsModal(false)}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-[2rem] w-full max-w-2xl shadow-2xl relative overflow-hidden p-8" onClick={(e) => e.stopPropagation()}>
                ...
                <table className="w-full text-sm text-center border-collapse">
                  <thead>
                    <tr className="bg-[var(--table-header)] text-white">
                      ...
                      <th className="p-4 border border-primary-hover">
                        {t("product")} ({t("code")})
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {invoiceItems.map((item, idx) => (
                      <tr key={idx} className="border-b border-gray-100">
                        ...
                        <td className="p-4 border-x border-gray-100 text-right">
                          {item.code} ({item.name})
                        </td>
                      </tr>
                    ))}
