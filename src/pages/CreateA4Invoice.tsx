...
const CreateA4Invoice: React.FC = () => {
    ...
    if (existing) {
      setItems(items.map(item => 
        item.productId === product.id 
          ? { ...item, qty: item.qty + 1, total: (item.qty + 1) * item.price * 1.15 } 
          : item
      ));
    } else {
  ...
  const handleUpdateQty = (id: string, qty: number) => {
    if (qty < 1) return;
    setItems(items.map(item => 
      item.id === id ? { ...item, qty, total: qty * item.price * 1.15 } : item
    ));
  };
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {categories.map(cat => (
            <button
              ...
              className={cn(
                "px-6 py-2 rounded-lg font-bold whitespace-nowrap transition-all",
                activeCategory === cat 
        ...
        <div className="flex-1 overflow-y-auto pr-2 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pb-4">
          {allProducts.map(product => (
            <motion.button
              ...
              className="bg-white rounded-xl border border-emerald-100 overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col"
            >
      ...
      <div className="w-[400px] bg-white rounded-2xl border border-emerald-100 shadow-lg flex flex-col overflow-hidden">
        ...
        <div className="p-4 border-b border-emerald-50">
          <div className="relative">
            <input
              type="text"
              placeholder="قراءة الباركود / ابحث عن طريق اسم أو الباركود"
              className="w-full bg-gray-50 border border-emerald-100 rounded-lg px-4 py-2.5 text-sm text-right outline-none focus:ring-2 focus:ring-emerald-100"
            />
                ...
                <tr key={item.id} className="border-b border-emerald-50 hover:bg-emerald-50/30">
                  ...
                  <td className="p-2 text-center">
                    <input
                      ...
                      className="w-12 border border-emerald-100 rounded px-1 py-0.5 text-center outline-none focus:border-emerald-500"
                    />
                  </td>
        ...
        <div className="p-4 bg-emerald-50/50 space-y-3">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-bold text-emerald-900">{items.length} ({items.reduce((s, i) => s + i.qty, 0)})</span>
          </div>
          <div className="flex justify-between text-sm">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button className="bg-emerald-600 text-white py-3 rounded-xl font-bold hover:bg-emerald-700 shadow-md transition-all active:scale-95">
              دفع (F9)
            </button>
            <button className="bg-orange-500 text-white py-3 rounded-xl font-bold hover:bg-orange-600 shadow-md transition-all active:scale-95">
              تعليق الفاتورة (F7)
            </button>
          </div>
        </div>
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="bg-white rounded-2xl border border-blue-100 shadow-sm overflow-hidden">
        <div className="bg-blue-600 p-4 flex items-center justify-between text-white">
          ...
          <div className="bg-white text-blue-600 px-6 py-1.5 rounded-lg font-black text-lg shadow-inner">
            فاتورة مبيعات
          </div>
           ...
           <div className="space-y-2 relative">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full border border-blue-200 rounded-xl px-4 py-2.5 text-right outline-none focus:ring-2 focus:ring-blue-100 shadow-sm"
                />
           ...
           <div className="space-y-2">
              ...
              <div className="flex">
                <input
                  ...
                  readOnly
                  value="0"
                  className="w-full border border-blue-200 rounded-r-xl px-4 py-2.5 text-center bg-gray-50 font-bold"
                />
                <span className="bg-blue-600 text-white px-4 py-2.5 rounded-l-xl flex items-center justify-center font-bold">مدين</span>
              </div>
           </div>
           <div className="flex items-end">
              <button className="w-full bg-blue-600 text-white py-2.5 rounded-xl font-bold hover:bg-blue-700 shadow-md transition-all active:scale-95 flex items-center justify-center gap-2">
                الحساب
              </button>
           </div>
        </div>
            ...
            {items.map(item => (
              <tr key={item.id} className="border-b border-blue-50 hover:bg-blue-50/30">
                <td className="p-3 border-l border-blue-50 text-center">
                   <button onClick={() => handleRemoveItem(item.id)} className="text-red-500">×</button>
                </td>
                ...
                <td className="p-3 border-l border-blue-50">
                   <input
                     ...
                     value={item.qty}
                     onChange={(e) => handleUpdateQty(item.id, parseInt(e.target.value) || 1)}
                     className="w-20 border border-blue-100 rounded px-2 py-1 text-center outline-none focus:border-blue-500"
                   />
                </td>
                <td className="p-3 border-l border-blue-50">{item.price.toFixed(2)}</td>
              ...
              <td colSpan={6} className="p-0">
                 <div className="relative">
                    <input
                      ...
                      className="w-full bg-transparent p-3 text-right outline-none placeholder:text-blue-300 pr-10"
                      value={searchTerm}
                    ...
                    {searchTerm && (
                      <div className="absolute top-full left-0 right-0 bg-white border border-blue-100 shadow-xl z-50 max-h-60 overflow-y-auto rounded-b-xl">
                        {allProducts.filter(p => p.name.includes(searchTerm)).map(p => (
                          <button
                            key={p.id}
                            onClick={() => {
                              handleAddItem(p);
      ...
      <div className="mb-6 flex items-center justify-between bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <select
            ...
            className="border border-gray-200 rounded-lg px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-100"
          >
            {warehouses.map(w => (
        ...
        <button 
          onClick={() => navigate('/sales/a4-invoices')}
