...
export default function Login() {
  ...
  const lang: Lang =
    (language as Lang) in translations ? (language as Lang) : "ar";
  ...
  return (
    ...
    >
      ...
      <nav className="relative z-20 flex items-center justify-between px-10 py-5">
        {/* Lang switcher */}
        <div
          className={`flex items-center gap-3 text-sm font-bold rounded-full px-5 py-2 border ${
            isDark
              ? "bg-white/5 border-white/10 text-slate-400"
              : "bg-white/70 border-black/10 text-slate-500"
          }`}
        >
            ...
            <React.Fragment key={l}>
              ...
              <button
                ...
                className={`transition-colors font-bold ${
                  lang === l ? "text-emerald-500" : "hover:text-emerald-500"
                }`}
              >
        ...
        <button
          ...
          className={`w-12 h-12 rounded-full flex items-center justify-center transition-all hover:bg-emerald-500/10 hover:text-emerald-500 ${
      ...
      <main className="relative z-10 flex-1 flex flex-col lg:flex-row rtl:lg:flex-row-reverse items-center justify-center gap-10 lg:gap-20 px-10 py-4 max-w-7xl mx-auto w-full overflow-hidden">
        ...
        <div className="w-full shrink-0" style={{ maxWidth: 420 }}>
          <div
            className={`rounded-[2.5rem] px-8 py-10 transition-all duration-300 ${
              isDark
                ? "bg-slate-800/92 border border-white/7 shadow-2xl"
                : "bg-white/95 border border-white shadow-2xl shadow-slate-300/40"
            }`}
          >
            <h1
            ...
            <p className="text-[10px] font-bold text-center text-slate-400 uppercase tracking-widest mb-8">
              {t.sub}
            </p>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  required
                  className={`w-full px-5 rounded-2xl text-sm border-2 outline-none transition-all focus:ring-[4px] focus:ring-emerald-500/10 focus:border-emerald-500 font-bold ${
                    isDark
                      ? "bg-slate-900 border-slate-700 text-slate-100 placeholder-slate-600"
                      : "bg-slate-50 border-slate-100 text-slate-800 placeholder-slate-300"
                  }`}
                  style={{ height: 50 }}
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  style={{ height: 50, letterSpacing: '0.2em' }}
                />
              </div>

              {error && (
                <p className="text-red-500 text-[10px] text-center py-1 font-black animate-pulse">
                  {error}
                </p>
              )}
              ...
              <div className="flex justify-between items-center py-1">
                ...
                <button
                  ...
                  className="text-[10px] font-black text-slate-400 hover:text-emerald-500 transition-colors uppercase tracking-wide"
                >
                  {t.forgot}
                </button>
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full rounded-2xl text-white text-sm font-black bg-[#00a651] hover:bg-[#009147] active:scale-[0.98] transition-all shadow-xl shadow-emerald-500/20 disabled:opacity-50 disabled:cursor-not-allowed mt-4 group flex items-center justify-center gap-3"
                style={{ height: 52 }}
              >
                {loading ? (
                  ...
                  <>
                    {t.loginBtn}
                    <div className="w-6 h-6 rounded-full bg-white/20 flex items-center justify-center group-hover:translate-x-1 transition-transform rtl:group-hover:-translate-x-1">
                      →
                    </div>
                  </>
                )}
      ...
      <footer
        className={`relative z-10 text-center font-black tracking-[0.4em] uppercase py-8 opacity-40 ${
          isDark ? "text-slate-400" : "text-slate-500"
        }`}
        style={{ fontSize: 9 }}
      >
        © 2026 Takamul Data Systems • Excellence & Integration
      </footer>
    </div>
  );
}
