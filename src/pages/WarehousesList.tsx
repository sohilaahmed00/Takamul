...
// import { useGetAllWarehouses, useDeleteWarehouse } from "../hooks/useWarehouses";
...
export default function WarehousesList() {
  ...
  const filteredData = useMemo(() => {
    ...
    return (warehouses ?? []).filter((item: any) => 
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      notifySuccess("تم حذف المخزن بنجاح");
    ...
    <div dir={direction}>
      <Card>
        <CardHeader>
          ...
          <CardDescription>إضافة وتعديل وحذف المخازن وربطها بالفروع</CardDescription>
          <CardAction>
            <Button onClick={() => { setSelectedId(undefined); setIsModalOpen(true); }}>
        ...
        <CardContent>
          ...
          >
            <Column field="warehouseCode" header="الكود" sortable />
            <Column field="warehouseName" header="اسم المخزن" sortable />
            <Column field="branchName" header="الفرع" />
            <Column field="managerName" header="المدير" />
            <Column
              header="الإجراءات"
              body={(row) => (
                <div className="flex gap-2">
                  <button onClick={() => { setSelectedId(row.id); setIsModalOpen(true); }} className="btn-minimal-action"><Edit2 size={16} /></button>
                  <button onClick={() => setDeleteTarget(row)} className="btn-minimal-action"><Trash2 size={16} /></button>
                </div>
              )}
            />
          </DataTable>
